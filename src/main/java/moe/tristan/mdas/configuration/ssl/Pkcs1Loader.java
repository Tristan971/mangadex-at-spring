...
final class Pkcs1Loader {
    ...
    private static final Pattern PKCS1_PATTERN = Pattern.compile(
        PKCS1_HEADER + BASE64_TEXT + PKCS1_FOOTER,
        Pattern.CASE_INSENSITIVE);

    private static final Pattern PKCS8_KEY_PATTERN = Pattern.compile(
        PKCS8_HEADER + BASE64_TEXT + PKCS8_FOOTER,
        Pattern.CASE_INSENSITIVE);
     ...
     *
     * @param text the private key content
     *
    ...
    public static PrivateKey parse(String text) {
        try {
            ...
            throw new IllegalStateException("Error loading private key", ex);
        }
    }
    ...
    private static byte[] convertPkcs1ToPkcs8(byte[] pkcs1) {
        try {
            ...
            result.write(bytes(0x30, 0x0D, 0x06, 0x09, 0x2A, 0x86, 0x48, 0x86, 0xF7, 0x0D, 0x01, 0x01, 0x01, 0x05, 0x00));
            // Octet string + length
